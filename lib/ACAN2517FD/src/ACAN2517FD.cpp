...
static const uint16_t TSCON_REGISTER    = 0x010 ;
static const uint16_t TBC_REGISTER      = 0x014 ;
...
//----------------------------------- Install interrupt, configure external interrupt
  ...
  //----------------------------------- 🔧 Configure TSCON (Time Stamp Control)
    // TBCEN (bit 16) = 1 (Enable TBC)
    // TSEOF (bit 17) = 1 (Time Stamp at EOF)
    // TBCPRE (bits 0-9) = prescaler for 1us resolution
    uint32_t sysClockHz = inSettings.mSysClock ;
    uint32_t tbcpre = (sysClockHz / 1000000) - 1 ;
    uint32_t tscon = (1UL << 16) | (1UL << 17) | (tbcpre & 0x3FF) ;
    writeRegister32 (TSCON_REGISTER, tscon) ;
    // 🔧 Reset TBC (Time Base Counter)
    writeRegister32 (TBC_REGISTER, 0) ;
  //----------------------------------- Configure TXQ
  ...
  //----------------------------------- Configure RX FIFO (FIFOCON, DS20005688B, page 52)
    ...
    data8 |= 1 << 5 ; // 🔧 Enable Receive Time Stamp (RXTSEN)
...
//--- Read word register via 6-byte buffer (speed enhancement, thanks to thomasfla)
  uint8_t buffer [78] = {0} ; // 🔧 Increased from 74 to 78 for Time Stamp
//--- Enter command
...
//--- SPI transfer
  assertCS () ;
    mSPI.transfer (buffer, 78) ; // 🔧 Read 78 bytes
  ...
  //--- 🔧 Read Time Stamp (enabled by RXTSEN)
    message.timestamp = u32FromBufferAtIndex (buffer, 10) ;
  //--- Write data (Swap data if processor is big endian)
    const uint32_t wordCount = (message.len + 3) / 4 ;
    for (uint32_t i=0 ; i < wordCount ; i++) {
      message.data32 [i] = u32FromBufferAtIndex (buffer, 14 + 4 * i) ; // 🔧 Data starts at offset 14
    }
  deassertCS () ;
//--- Increment FIFO
...
uint32_t ACAN2517FD::currentTBC (void) {
  return readRegister32 (TBC_REGISTER) ;
}

void ACAN2517FD::setTBC (uint32_t value) {
  writeRegister32 (TBC_REGISTER, value) ;
}

void ACAN2517FD::pauseTBC (void) {
  uint32_t tscon = readRegister32 (TSCON_REGISTER) ;
  tscon &= ~(1UL << 16) ; // Clear TBCEN
  writeRegister32 (TSCON_REGISTER, tscon) ;
}

void ACAN2517FD::resumeTBC (void) {
  uint32_t tscon = readRegister32 (TSCON_REGISTER) ;
  tscon |= (1UL << 16) ; // Set TBCEN
  writeRegister32 (TSCON_REGISTER, tscon) ;
}
