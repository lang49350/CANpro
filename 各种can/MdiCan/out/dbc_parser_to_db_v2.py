...
class DBCParser:
    ...
    def insert_messages(self, dbc_metadata_id) -> None:
        """插入消息数据到数据库"""
        ...
        try:
            for message in self.dbc.messages:
                ...
                self.cursor.execute(
                    ...
                    (message_id, name, length, cycle_time, send_type, sender, dbc_comment, user_comment, dbc_metadata_id)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)''',
                    (
                        hex_message_id,
                        message.name,
    ...
    def insert_signals(self, dbc_metadata_id) -> None:
        """插入信号数据到数据库"""
        if not self.dbc or not self.cursor:
            return
            ...
            for message in self.dbc.messages:
                ...
                for signal in message.signals:
                    ...
                    self.cursor.execute(
                        ...
                        scale, offset, minimum, maximum, unit, receiver, dbc_comment, user_comment, dbc_metadata_id)
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''',
                        params
                    )
                    ...
                    if signal.choices:
                        for value, description in signal.choices.items():
                            ...
                            self.cursor.execute(
                                ...
                                (signal_id, value, description, dbc_metadata_id) VALUES (?, ?, ?, ?)''',
                                (signal_id, value, description_str, dbc_metadata_id)
                            )
            self.conn.commit()
        except Exception as e:
            print(f"插入信号数据失败: {e}")
    ...
    def insert_nodes(self, dbc_metadata_id) -> None:
        """插入节点数据到数据库"""
        if not self.dbc or not self.cursor:
            return
        ...
        try:
            for node in self.dbc.nodes:
                self.cursor.execute(
                    '''INSERT INTO nodes (name, dbc_comment, user_comment, dbc_metadata_id) VALUES (?, ?, ?, ?)''',
                    (node.name, node.comment if node.comment else '', '', dbc_metadata_id)
                )
            self.conn.commit()
        except Exception as e:
            print(f"插入节点数据失败: {e}")
    ...
    def insert_environment_variables(self, dbc_metadata_id) -> None:
        """插入环境变量数据到数据库"""
        ...
        try:
            for env_var in self.dbc.environment_variables:
                self.cursor.execute(
                    ...
                    (name, type, minimum, maximum, unit, initial_value, access_type, dbc_comment, user_comment, dbc_metadata_id)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''',
                    (
                        env_var.name,
                        env_var.type,
                ...
                if env_var.data_length is not None:
                    self.cursor.execute(
                        '''INSERT INTO env_var_data_lengths (env_var_id, data_length, dbc_metadata_id) VALUES (?, ?, ?)''',
                        (env_var_id, env_var.data_length, dbc_metadata_id)
                    )

                # 插入环境变量接收器
                for receiver in env_var.receivers:
                    self.cursor.execute(
                        '''INSERT INTO env_var_receivers (env_var_id, node_name, dbc_metadata_id) VALUES (?, ?, ?)''',
                        (env_var_id, receiver, dbc_metadata_id)
                    )
            self.conn.commit()
        except Exception as e:
    ...
    def insert_attribute_definitions(self, dbc_metadata_id) -> None:
        """插入属性定义数据到数据库"""
        ...
        try:
            for attr_def in self.dbc.attribute_definitions:
                self.cursor.execute(
                    ...
                    (name, type, default_value, min_value, max_value, dbc_metadata_id)
                    VALUES (?, ?, ?, ?, ?, ?)''',
                    (
                        attr_def.name,
                        attr_def.type,
    ...
    def insert_attributes(self, dbc_metadata_id) -> None:
        """插入属性数据到数据库"""
            ...
            for message_id, attributes in self.dbc.attributes.get('messages', {}).items():
                ...
                for attr_name, attr_value in attributes.items():
                    self.cursor.execute(
                        ...
                        (message_id, attribute_name, value, dbc_metadata_id) VALUES (?, ?, ?, ?)''',
                        (hex_message_id, attr_name, attr_value, dbc_metadata_id)
                    )
                    ...
                    if signal:
                        ...
                        for attr_name, attr_value in attributes.items():
                            self.cursor.execute(
                                ...
                                (signal_id, attribute_name, value, dbc_metadata_id) VALUES (?, ?, ?, ?)''',
                                (signal_id, attr_name, attr_value, dbc_metadata_id)
                            )
                ...
                if node:
                    ...
                    for attr_name, attr_value in attributes.items():
                        self.cursor.execute(
                            ...
                            (node_id, attribute_name, value, dbc_metadata_id) VALUES (?, ?, ?, ?)''',
                            (node_id, attr_name, attr_value, dbc_metadata_id)
                        )
    ...
    def parse_and_store(self) -> bool:
        ...
        self.create_tables()
        dbc_metadata_id = self.insert_metadata()
        if dbc_metadata_id == -1:
            return False

        self.insert_messages(dbc_metadata_id)
        self.insert_signals(dbc_metadata_id)
        self.insert_nodes(dbc_metadata_id)
        self.insert_environment_variables(dbc_metadata_id)
        self.insert_attribute_definitions(dbc_metadata_id)
        self.insert_attributes(dbc_metadata_id)
        self.close()
        return True
...
if __name__ == "__main__":
    ...
    if len(sys.argv)>2:
        ...
        print("参数错误，参数1=DBC文件地址,参数2=数据库地址")
    # 使用示例
    # dbc_file_path = file_path # 替换为实际的 DBC 文件路径
    # sqlite_db_path = 'dbc.db'  # 替换为实际的 SQLite 数据库路径
    parser = DBCParser(file_path, sqlite_db_path)
